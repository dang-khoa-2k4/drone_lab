            ...
            if mode == "GUIDED":
                ...
                if payload["header"] == "servo":
                    if payload['data'] == 'open':
                        ...
                        # try:
                        #     subprocess.run(['python3', './control.py', "open"], capture_output=True, text=True)
                        # except:
                        #     pass
                    elif payload['data'] == 'close':
                        self.servo.close_handle()
                        # try:
                        #     subprocess.run(['python3', './control.py', "close"], capture_output=True, text=True)
                        # except:
                        #     pass
                   # time.sleep(1)
    ...
    def processing_2(self):
        while self.running:
            if self.frame is not None:
               ...
               # data2 =self.ob_qr.object_detect(self.frame)
               # print(f"          {data2}")
